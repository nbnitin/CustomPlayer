...
let CONTROL_HEIGHT_IPAD : CGFloat = 210
let CONTROL_HEIGHT_IPHONE : CGFloat = 105
class AudioViewControllerNewView: UIViewController, AudioDisplayProtocol {
    
    //outlets
    @IBOutlet var videoLoaderViewContainer: UIView!
    ...
    @IBOutlet weak var videoSlider: CustomSliderForNewView!
    ...
    override func viewDidLoad() {
        ...
        controlContainerHeightConstraint.constant = AppUtility.isPad() ? CONTROL_HEIGHT_IPAD : CONTROL_HEIGHT_IPHONE
        let videoSliderHeightToBe = controlContainerHeightConstraint.constant
        ...
        oldVideoControlHeightConstraintConstant = controlContainerHeightConstraint.constant
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        controlContainerHeightConstraint.constant = oldVideoControlHeightConstraintConstant
...
extension AudioViewControllerNewView: VideoEventDelegate {
    
    func startLoadingWhenVideoBuffering(show:Bool){
        
        if show {
            self.videoLoaderViewContainer.isHidden = false
            (self.videoLoaderViewContainer.subviews.last as? UIActivityIndicatorView)?.startAnimating()
        } else {
            self.videoLoaderViewContainer.isHidden = true
            (self.videoLoaderViewContainer.subviews.last as? UIActivityIndicatorView)?.startAnimating()
        }
    }
    ...
    func videoPlayerDidUpdatePlaying(isVideoLoading: Bool) {
        ...
        startLoadingWhenVideoBuffering(show: isVideoLoading)
    ...
    func videoPlayerDidUpdateTime(currentTime: Int) {
        ...
        if playPauseButton.isHidden || !videoLoaderViewContainer.isHidden  {
            playPauseButton.isEnabled = true
            startLoadingWhenVideoBuffering(show: false)
        }
...
//            self.brandImageBottomConstraint.constant = -(oldVideoControlHeightConstraintConstant)
            ...
            self.controlContainerHeightConstraint.constant = oldVideoControlHeightConstraintConstant
